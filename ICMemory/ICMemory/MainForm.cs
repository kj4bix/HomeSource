...
namespace ICMemory
{
	public partial class MainForm : Form
	{
		...
		public MainForm()
		{
			...
			//this.Address = "70"; // Icom 7000
			this.Address = "6E"; // 756 Pro III
			this.PreFix = @"FE FE " + this.Address + " ";
		}
		...
		private void btnWriteMemory_Click(object sender, EventArgs e)
		{
			...
			for (int iMem=1; iMem<=5; iMem++)
			{
				...
				sCommand = this.PreFix + "E0 08 A0 " + iMem.ToString("D2") + " FD";
				...
				for (int i=1; i<= 99; i++)
				{
					sCommand = this.PreFix + "E0 08 00 " + i.ToString("D2") + " FD";
					this.CurrentResponseType = ResponseType.FB;
					this.oComm.WriteData(sCommand);
					sResult = this.WaitResponse();

					// Clear what is there
					sCommand = this.PreFix + "E0 0B FD";
					this.CurrentResponseType = ResponseType.FB;
					this.oComm.WriteData(sCommand);
					sResult = this.WaitResponse();
		...
		private void btnReadMemory_Click(object sender, EventArgs e)
		{
			...
			string sCommand = string.Empty;
			int iMemories = 1;
			if (this.Address == "70") iMemories = 5;
			for (int iMem=1; iMem <= iMemories; iMem++) // 1, 2, 3, 4, 5 for A-E (for the 7000)
			{
				// Switch to Memory Bank
				...
				for (int i = 1; i <= 99; i++)
				{
					...
					// Tone
					sQuery = this.PreFix + "E0 1B 00 FD";
					this.CurrentResponseType = ResponseType.VAL;
					this.oComm.WriteData(sQuery);
					sResult = this.WaitResponse();
